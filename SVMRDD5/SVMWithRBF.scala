...
class SVMWithRBF(var input: RDD[LabeledPoint], var C: Double, var eps: Double, var tolerance: Double, var gamma: Double, var test: RDD[LabeledPoint]) extends Serializable {
  ...
  val trainMatrix = data.map { case (label, features) => // 列存储方式的训练样本矩阵(column-major)
  ...
  def takeStep(i1: Int, i2: Int): Int = {
    ...
    val eta = 2 * k12 - k11 - k22 // val eta = k11 + k22 - 2 * k12
    ...
    a1 = alpha1 - s * (a2 - alpha2) // a1 = alpha1 + y1 * y2 * (alpha2 - a2) // 计算新的a1
    if(a1 < 0) { // 调整a1，使其符合条件
      a2 += s * a1
      a1 = 0
    } else if(a1 > C) {
  ...
  def auROC(data: RDD[(Double, DoubleMatrix)]) = {
    ...
    val n = data.count().toDouble
    println("测试数据的个数为： " + n)
    val scoreAndLabels = data.map { case (label, features) =>
      val score = learnedFun2(features)
      (score, label)
    }
  ...
  /**
   * 创建模型
   * (alpha: Double, supportVectors: DoubleMatrix)
   */
  def createModel(trainMatrix: Array[DoubleMatrix], b: Double, gamma: Double): SVMModel = {
    var supportVectorsWithAlpha = ArrayBuffer[(Double, Double, DoubleMatrix)]()
    for(i <- 0 until trainNum) {
      if(alpha.get(i) > 0 && alpha.get(i) <= C) {
        val tuple = (alpha.get(i), label(i), trainMatrix(i))
        supportVectorsWithAlpha += tuple
      }
    }
    new SVMModel(supportVectorsWithAlpha.toArray, b, gamma)
  }
